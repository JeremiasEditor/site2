...
export default function Contact() {
  ...
  return (
    <section id="contato" className="relative py-32 overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom,rgba(255,27,27,0.06)_0%,transparent_60%)]" />
      ...
      <div ref={ref} className="relative z-10 max-w-3xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-bold mt-4 mb-6">
            {contact.headingLead}{" "}
            <span className="gradient-text-red">{contact.headingHighlight}</span>
          </h2>
        </motion.div>

        <div className="space-y-6">
          {contactInfo.map((info, i) => {
            const InfoIcon = contactIconMap[info.icon] ?? Mail;
            return (
            <motion.div
              key={i}
            ...
            >
              {info.href ? (
                <a
                  ...
                  className="group flex items-center gap-4 glass rounded-xl p-5 hover:border-primary/20 transition-all duration-300"
                >
          ...
          >
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-3 w-full py-4 bg-green-500/10 text-green-400 border border-green-500/20 rounded-xl font-medium hover:bg-green-500 hover:text-white transition-all duration-300"
            >
